...
public final class DiamondBlockPillarSurprise implements Surprise {
    ...
    public void activate(@Nonnull Random random, @Nonnull Player p, @Nonnull Location l) {
        for (int i = 0; i < 8; i++) {
            LuckyBlockCompat.spawnFallingBlock(l.getWorld(), l.clone().add(0.5, (i + 1) * 4.0, 0.5), ColoredMaterial.TERRACOTTA.get(i));
        }

        LuckyBlockCompat.spawnFallingBlock(l.getWorld(), l.clone().add(0.5, 9 * 4.0, 0.5), LuckyBlockCompat.safe(XMaterial.DIAMOND_BLOCK));
    }
